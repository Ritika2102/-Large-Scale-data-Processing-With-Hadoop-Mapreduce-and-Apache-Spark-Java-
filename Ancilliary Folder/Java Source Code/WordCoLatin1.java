...
public class WordCoLatin1 {
    	...
    	String arr[]=remaining.split(" ");
    	 for(int j=0;j<arr.length-1;j++){
	    	...
	    	for(int k=i+1;k<arr.length;k++){
	    		String neighbor1 = arr[k];
	    		String str2="";
	    		String twostr2="";
	    		str2 = neighbor1.replace("j", "i");
	    		twostr2=str2.replace("v", "u");
	    		String finalgoing=twostr+","+twostr1+","+twostr2;
  ...
  public static class IntSumReducer extends Reducer <Text,Text,Text,Text> {
    ...
    public void reduce(Text key, Iterable<Text> values,
      ...
      if(keyscome.length>2){
      String part1=keyscome[0];
      String part2= keyscome[1];
      String part3 = keyscome[2];
    	String going = "";
    	String keyToWrite = "";
    
        	if(LemmaMap.containsKey(part1)){
        		if(LemmaMap.containsKey(part2)){
        			if(LemmaMap.containsKey(part3)){
        				keyToWrite="<"+LemmaMap.get(part1)+","+LemmaMap.get(part2)+","+LemmaMap.get(part3)+">";
                		for (Text val : values) 
                        { 
        				...
        				keyToWrite="<"+LemmaMap.get(part1)+","+LemmaMap.get(part2)+","+part3+">";
                		for (Text val : values) 
                        { 
                              going = going+val.toString(); 
                 ...
                 else if(LemmaMap.containsKey(part3)){
                	 keyToWrite="<"+LemmaMap.get(part1)+","+part2+","+LemmaMap.get(part3)+">";
             		for (Text val : values) 
                     { 
                           going = going+val.toString(); 
        		...
        		if(LemmaMap.containsKey(part3)){
        			keyToWrite="<"+part1+","+LemmaMap.get(part2)+","+LemmaMap.get(part3)+">";
            		for (Text val : values) 
                    { 
                          going = going+val.toString(); 
        			...
        			keyToWrite="<"+part1+","+LemmaMap.get(part2)+","+part3+">";
            		for (Text val : values) 
                    { 
                          going = going+val.toString(); 
        		...
        		keyToWrite="<"+part1+","+part2+","+LemmaMap.get(part3)+">";
        		for (Text val : values) 
                { 
                      going = going+val.toString(); 
  ...
  public static void main(String[] args) throws Exception {
   ...
   // FSDataInputStream inputStream = fs.open(new Path("C:/Users/Fnu/Downloads/la.lexicon.csv"));
    ...
    job.setJarByClass(WordCoLatin1.class);
